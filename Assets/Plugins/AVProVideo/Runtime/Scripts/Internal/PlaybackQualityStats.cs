...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class PlaybackQualityStats
	{
		...
		internal void Start(BaseMediaPlayer player)
		{
			...
			if (vsyncEnabled)
			{
				float vsyncRate = (float)Math.Round(Screen.currentResolution.refreshRateRatio.value) / QualitySettings.vSyncCount;
		...
		internal void Update()
		{
			...
			if (QualitySettings.vSyncCount != 0)
			{
				long vsyncDuration = (long)((QualitySettings.vSyncCount * Helper.SecondsToHNS) / (float)Math.Round(Screen.currentResolution.refreshRateRatio.value) );
