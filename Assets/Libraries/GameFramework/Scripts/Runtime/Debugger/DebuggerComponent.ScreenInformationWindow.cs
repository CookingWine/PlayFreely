...
namespace UnityGameFramework.Runtime
{
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        private sealed class ScreenInformationWindow : ScrollableDebuggerWindowBase
        {
            ...
            {
                int refresh = (int)Math.Round(resolution.refreshRateRatio.value);
                return Utility.Text.Format("{0} x {1} @ {2}Hz", resolution.width.ToString(), resolution.height.ToString(), refresh.ToString());
            }
