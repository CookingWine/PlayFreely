...
namespace PlayFreely.HotfixRuntime
{
    ...
    public static class DataTableExtension
    {
        ...
        public static int4 Parseint4(string value)
        {
            ...
            string[] splitValue = value.Split(',');
            return new int4(int.Parse(splitValue[0]) , int.Parse(splitValue[1]) , int.Parse(splitValue[2]) , int.Parse(splitValue[3]));
        }
        ...
        public static int4[] Parseint4Array(string value)
        {
            if(string.IsNullOrWhiteSpace(value)) return null;
            string[] arr = ParseArrayElements(value);
            int4[] result = new int4[arr.Length];
            for(int i = 0; i < arr.Length; i++)
            {
        ...
        public static int4[] Readint4Array(this BinaryReader binaryReader)
        {
            int length = binaryReader.Read7BitEncodedInt32( );
            int4[] result = new int4[length];
            for(int i = 0; i < length; i++)
            {
