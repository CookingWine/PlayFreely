...
namespace PlayFreely.HotfixRuntime
{
    ...
    public sealed class Fsm:FsmBase, IFsm
    {
        ...
        public Fsm(string name , object owner , params FsmState[] states) : base(name)
        {
            ...
            m_Owner = owner ?? throw new GameFrameworkException("FSM owner is invalid.");
            ...
            foreach(FsmState state in states)
            {
                ...
                {
                    throw new GameFrameworkException(Utility.Text.Format("FSM '{0}' state '{1}' is already exist." , name , stateName));
                }
        ...
        public void Start<TState>( ) where TState : FsmState
        {
            ...
            FsmState state = GetState<TState>( ) ?? throw new GameFrameworkException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist." , Name , typeof(TState).FullName));
            m_CurrentStateTime = 0f;
        ...
        public void Start(Type stateType)
        {
            ...
            FsmState state =  GetState(stateType)  ?? throw new GameFrameworkException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist." , Name , stateType.FullName));
            m_CurrentStateTime = 0f;
            m_CurrentState = state;
            m_CurrentState.OnEnter(this);
        }
        ...
        {
            if(m_States.TryGetValue(typeof(TState).FullName , out FsmState state))
            {
                return (TState)state;
            }
        ...
        public FsmState GetState(Type stateType)
        {
            ...
            if(m_States.TryGetValue(stateType.FullName , out FsmState state))
            {
                return state;
            }
        ...
        public Variable GetData(string name)
        {
            ...
            if(m_Datas.TryGetValue(name , out Variable data))
            {
                return data;
            }
        ...
        internal void ChangeState(Type stateType)
        {
            ...
            FsmState state =  GetState(stateType)  ?? throw new GameFrameworkException(Utility.Text.Format("FSM '{0}' can not change state to '{1}' which is not exist." , Name , stateType.FullName));
